...
const authenitication = (request, response, next) => {
  let jwtToken
  const authHeader = request.headers['authorization']
  if (authHeader) {
    jwtToken = authHeader.sqlit(' ')[1]
  }

  if (jwtToken) {
    jwt.verify(jwtToken, 'SECRER_KEY', (error, payload) => {
      if (error) {
        response.status(401)
...
app.post('/register/', async (request, response) => {
  ...
  if (userDBDetails !== undefined) {
    ...
    if (password.length < 6) {
      ...
      const createUserQuery = `INSERT INTO username, password,name,gender}
...
app.post('/Login/', async (request, response) => {
  ...
  if (userDbDetails !== undefined) {
    ...
    if (isPasswordCorrect) {
      ...
      const jwtToken = just.sign(payload, 'SECRET_KEY')
...
app.get('/user/tweets/feed/', authenitication, async (request, response) => {
...
app.get('/user/following/', authenitication, async (request, response) => {
...
app.get('/user/followers/', authenitication, async (request, response) => {
  const {username, userId} = request
...
app.get(
  ...
  async (request, response) => {
    ...
    (SELECT COUNT() FROM like WHERE tweet id = '${tweetId}') As likes,
...
app.get('/user/tweetS/', authenitication, async (request, response) => {
  const {userId} = request
...
app.post('/user/tweets/', authenitication, async (request, response) => {
...
app.delete('/tweets/:tweetId/', authenitication, async (request, response) => {
  const {tweetId} = request.params
  const {userId} = request
